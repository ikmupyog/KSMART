...
const getRow = ({ id, service, complaintType, workState }) => {
  const status = workState?.state?.applicationStatus || service.applicationStatus
  const basicDetails = {
    CS_COMPLAINT_DETAILS_COMPLAINT_NO: id,
    CS_COMPLAINT_DETAILS_APPLICATION_STATUS: `CS_COMMON_${status}`,
    CS_ADDCOMPLAINT_COMPLAINT_TYPE: complaintType === "" ? `SERVICEDEFS.OTHERS` : `SERVICEDEFS.${complaintType}`,
    CS_ADDCOMPLAINT_COMPLAINT_SUB_TYPE: `SERVICEDEFS.${service.serviceCode.toUpperCase()}`,
    CS_COMPLAINT_FILED_DATE: Digit.DateUtils.ConvertTimestampToDate(service.auditDetails.createdTime)
  }
  const additionalDetails = {
    CS_COMPLAINT_ADDTIONAL_DETAILS: service.description,
    ES_CREATECOMPLAINT_ADDRESS: [service.address.landmark,
    Digit.Utils.locale.getLocalityCode(service.address.locality, service.tenantId),
    service.address.city, service.address.pincode]
  }

  return { basicDetails, additionalDetails }
};

const isEmptyOrNull = (obj) => obj === undefined || obj === null || Object.keys(obj).length === 0;

const transformDetails = ({ id, service, workflow, thumbnails, complaintType, workState }) => {
...
const fetchComplaintDetails = async (tenantId, id) => {
  ...
  const workState = (await Digit?.WorkflowService?.getByBusinessId(tenantId, id)).ProcessInstances[0] || {}
  ...
  if (service && workflow && serviceDefs) {
    ...
    const details = transformDetails({ id, service, workflow, thumbnails, complaintType, workState });
    return details;
  } else {
    return {};
  }
