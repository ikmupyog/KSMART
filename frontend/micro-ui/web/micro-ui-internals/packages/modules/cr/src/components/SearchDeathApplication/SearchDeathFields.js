...
const SearchDeathFields = ({ register, control, reset, tenantId, previousPage, t }) => {
  ...
  const { data: hospitalData = {}, isLoading: hospitalLoading } = Digit.Hooks.cr.useCivilRegistrationMDMS(
    "kl.cochin",
    "cochin/egov-location",
    "hospital"
  );
    ...
    <React.Fragment>
      ...
      <SearchField>
        ...
        <TextInput
          ...
          {...(validation = {
            ...
            isRequired: false,
            type: "text",
            title: t("TEMPLATE_INVALID", { type: t("DC_NAME_DECEASED") }),
          })}
        />
      </SearchField>
      <SearchField>
        <label>{t("CR_DATE_OF_DEATH")}</label>
        <Controller
          render={(props) => (
            <DatePicker
              date={props.value}
              onChange={props.onChange}
              {...(validation = { pattern: "[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}", isRequired: false, title: t("CR_DATE_OF_DEATH") })}
            />
          )}
          name="DeathDate"
          control={control}
      ...
      <SearchField>
        ...
        <TextInput
          name="FatherNameEn"
          inputRef={register({})}
          placeholder={`${t("DC_DECEASED_FATHER_NAME")}`}
      ...
      <SearchField>
        ...
        <TextInput
          name="MotherNameEn"
          inputRef={register({})}
          placeholder={`${t("DC_DECEASED_MOTHER_NAME")}`}
      ...
      <SearchField>
        ...
        <TextInput
          name="AddressEn"
          inputRef={register({})}
          placeholder={`${t("DC_DECEASED_ADDRESS")}`}
          {...(validation = { pattern: "^[a-zA-Z-.`' ]*$", isRequired: false, type: "text", title: t("DC_INVALID_ADDRESS") })}
        />
      </SearchField>
      <SearchField>
        <label style={{ marginTop: "20px" }}>{`${t("CR_HOSPITAL_EN")}`}</label>
        <Controller
          control={control}
          name="hospital"
          render={(props) => (
      ...
      <SearchField className="submit">
        ...
        <SubmitBar style={{ marginTop: "30px" }} label={t("ES_COMMON_SEARCH")} submit />
      </SearchField>
    </React.Fragment>
  );
};
