...
const BrideAddressPresentOutsideIndia = ({ config, onSelect, userType, formData, presentOutSideIndiaAdressEn, setAdressEn,
  ...
  if (isEditBirth) {
    ...
    if (formData?.BrideAddressDetails?.presentOutSideIndiaadrsVillage != null) {
      if (cmbUrbanRural.length > 0 && (presentOutSideIndiaadrsVillage === undefined || presentOutSideIndiaadrsVillage === "")) {
        setadrsVillage(cmbUrbanRural.filter(cmbUrbanRural => cmbUrbanRural.code === formData?.BrideAddressDetails?.presentOutSideIndiaadrsVillage)[0]);
      }
    }
...
export default BrideAddressPresentOutsideIndia;