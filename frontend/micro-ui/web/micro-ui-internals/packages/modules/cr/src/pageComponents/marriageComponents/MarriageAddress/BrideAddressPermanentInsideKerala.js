...
const BrideAddressPermanentInsideKerala = ({ config, onSelect, userType, formData,
  ...
  useEffect(() => {
    ...
    if (isInitialRender && isPrsentAddress && countryValuePermanent === "IND" && valuePermanent === "kl"
      && (formData?.BrideAddressDetails?.presentInsideKeralaLBName === null || formData?.BrideAddressDetails?.presentInsideKeralaLBName === "" || formData?.BrideAddressDetails?.presentInsideKeralaLBName === undefined)) {
     ...
     && (formData?.BrideAddressDetails?.permntInKeralaAdrLBName === null || formData?.BrideAddressDetails?.permntInKeralaAdrLBName === "" || formData?.BrideAddressDetails?.permntInKeralaAdrLBName === undefined)) {
  ...
  if (isEditBirth) {
    ...
    if (formData?.BrideAddressDetails?.permntInKeralaAdrDistrict != null) {
      if (cmbDistrict.length > 0 && (permntInKeralaAdrDistrict === undefined || permntInKeralaAdrDistrict === "")) {
        cmbFilterDistrict = cmbDistrict.filter((cmbDistrict) => cmbDistrict.statecode === valuePermanent);
        setDistrictPermvalue(cmbFilterDistrict);
        setpermntInKeralaAdrDistrict(cmbDistrict.filter(cmbDistrict => cmbDistrict.code === formData?.BrideAddressDetails?.permntInKeralaAdrDistrict)[0]);
      }
    }
    ...
    if (formData?.BrideAddressDetails?.permntInKeralaAdrVillage != null) {
      if (cmbVillage.length > 0 && (permntInKeralaAdrVillage === undefined || permntInKeralaAdrVillage === "")) {
        cmbFilterVillage = cmbVillage.filter((cmbVillage) => cmbVillage.distId === currentLB[0].city.districtid);
        setLbsVillagePermvalue(cmbFilterVillage);
        setpermntInKeralaAdrVillage(cmbVillage.filter(cmbVillage => cmbVillage.code === formData?.BrideAddressDetails?.permntInKeralaAdrVillage)[0]);
      }
    }
    ...
    if (formData?.BrideAddressDetails?.permntInKeralaAdrPostOffice != null) {
      if (cmbPostOffice.length > 0 && (permntInKeralaAdrPostOffice === undefined || permntInKeralaAdrPostOffice === "")) {
        setPostOfficePermvalues(cmbPostOffice.filter((cmbPostOffice) => cmbPostOffice.distid === currentLB[0].city.districtid));
        setpermntInKeralaAdrPostOffice(cmbPostOffice.filter(cmbPostOffice => cmbPostOffice.code === formData?.BrideAddressDetails?.permntInKeralaAdrPostOffice)[0]);
