...
function ApplicationContent({
  ...
  const fetchImage = async (uploadedImages) => {
    ...
    const newThumbnails = fileStoreIds.map((key) => {
      ...
      return {
        large: fileType === "image" ? key.url.split(",")[1] : key.url,
        small: fileType === "image" ? key.url.split(",")[2] : key.url,
        key: key.id,
        type: fileType,
        pdfUrl: key.url,
      };
      ...
      <div className="file-main">
        <div className={"cr-wrapper-app"}>
          {applicationDetails?.applicationDetails?.length > 0 &&
            applicationDetails?.applicationDetails?.map((detail, index) => renderSummaryCard(detail, index))}
        </div>
        <div className={"cr-timeline-wrapper"}>
          {imagesThumbs?.length > 0 && (
            <Carousel
              {...{ carouselItems: imagesThumbs }}
              imageHeight={300}
                          ...
                          return (
                            <React.Fragment key={index}>
                              <CheckPoint
                                ...
                                label={t(`${timelineStatusPrefix}${checkpoint?.[statusAttribute]}`)}
