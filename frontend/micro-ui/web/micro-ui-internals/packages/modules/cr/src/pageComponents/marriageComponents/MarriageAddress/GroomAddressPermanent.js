...
const GroomAddressPermanent = ({ config, onSelect, userType, formData, permtaddressCountry, setpermtaddressCountry,
    ...
    useEffect(() => {
        if (isPrsentAddress && isInitialRender) {
            ...
            (formData?.GroomAddressDetails?.permtaddressStateName === null || formData?.GroomAddressDetails?.permtaddressStateName === "" || formData?.GroomAddressDetails?.permtaddressStateName === undefined)) {
            if (cmbLB.length > 0) {
                currentLB = cmbLB.filter((cmbLB) => cmbLB.code === tenantId);
                // setAdrsLBName(currentLB[0]);
                if (cmbCountry.length > 0 && currentLB.length > 0) {
    ...
    if (isEditBirth) {
        ...
        if (formData?.GroomAddressDetails?.permtaddressCountry != null) {
            if (cmbCountry.length > 0 && (permtaddressCountry === undefined || permtaddressCountry === "")) {
                cmbFilterCountry = cmbCountry.filter(cmbCountry => cmbCountry.code === formData?.GroomAddressDetails?.permtaddressCountry);
                if (cmbFilterCountry.length > 0) {
                    setpermtaddressCountry(cmbFilterCountry[0]);
                    setCountryValuePermanent(cmbFilterCountry[0].countrycode);
                }
        ...
        if (formData?.GroomAddressDetails?.permtaddressStateName != null) {
            if (cmbState.length > 0 && (permtaddressStateName === undefined || permtaddressStateName === "")) {
                ...
                setValuePermanent(formData?.GroomAddressDetails?.permtaddressStateName);
            }
        }
