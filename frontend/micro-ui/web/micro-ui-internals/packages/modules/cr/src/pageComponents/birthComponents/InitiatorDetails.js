...
const InitiatorDetails = ({ config, onSelect, userType, formData,
    ...
    function setSelectinitiatorNameEn(e) {
        if (e.target.value.length === 51) {
            ...
            setinitiatorNameEn(e.target.value.replace(/^^[\u0D00-\u0D7F\u200D\u200C .&'@' 0-9]/ig, ''));
        }
    }
    function setSelectinitiatorDesi(e) {
        if (e.target.value.length === 51) {
            ...
            setinitiatorDesi(e.target.value.replace(/^^[\u0D00-\u0D7F\u200D\u200C .&'@' 0-9]/ig, ''));
        }
    }
    function setSelectinitiatorAddress(e) {
        if (e.target.value.length === 251) {
            ...
            setinitiatorAddress(e.target.value.replace(/^^[\u0D00-\u0D7F\u200D\u200C .&'@' 0-9]/ig, ''));
        }
    }
    ...
    const goNext = () => {
      ...
      // }
        ...
        onSelect(config.key, {
          relation , initiatorNameEn,  initiatorAadhar,   initiatorMobile ,initiatorDesi,  initiatorAddress,isInitiatorDeclaration ,isCaretaker });
        }
      };
    return (
        <React.Fragment>
          ...
          <div className="col-md-12">
            <CheckBox label={t("CR_INITIATOR_DECLARATION_STATEMENT")} onChange={setDeclarationInfo} value={isInitiatorDeclaration} checked={isInitiatorDeclaration} />
          </div>
        </div>
        <div className="row">
          <div className="col-md-12">
              ...
              <span style={{ background: "#fff", padding: "0 10px" }}>{`${t("CR_INITIATOR_PARENTS_GUARDIAN_CARETAKER")}`}</span>{" "}
                ...
                <div className="row">
                    ...
                    <div className="col-md-4" ><CardLabel>{`${t("CS_COMMON_AADHAAR")}`}<span className="mandatorycss">*</span></CardLabel>
                            <TextInput t={t} isMandatory={true} type={"number"} optionKey="i18nKey" name="initiatorAadhar" value={initiatorAadhar} onChange={setSelectinitiatorAadhar} disable={isEdit} 
                        ...
                        <div className="col-md-4" ><CardLabel>{`${t("CR_INFORMANT_NAME")}`}<span className="mandatorycss">*</span></CardLabel>
                            <TextInput t={t} isMandatory={true} type={"text"} optionKey="i18nKey" name="initiatorNameEn"
                                value={initiatorNameEn} onChange={setSelectinitiatorNameEn} disable={isEdit} placeholder={`${t("CR_INFORMANT_NAME")}`}
        ...
        {isCaretaker === true && (
          <div>
                        ...
                        <div className="col-md-3" ><CardLabel>{`${t("CR_MOBILE_NO")}`}<span className="mandatorycss">*</span></CardLabel>
                            <TextInput t={t} isMandatory={true} type={"number"} optionKey="i18nKey" name="initiatorMobile" value={initiatorMobile} onChange={setSelectinitiatorMobile} disable={isEdit} 
