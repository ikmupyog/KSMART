...
const AddressPermanentInsideKerala = ({
  config,
  onSelect,
  userType,
  formData,
  ...
  useEffect(() => {
    ...
    if (isInitialRender) {
      console.log("Hi");
      if (cmbLB.length > 0) {
        console.log("Hi", Digit.ULBService.getStateId())
        currentLB = cmbLB.filter((cmbLB) => cmbLB.code === tenantId);
        setpermntInKeralaAdrLBName(currentLB[0]);
        cmbFilterDistrict = cmbDistrict.filter((cmbDistrict) => cmbDistrict.statecode === Digit.ULBService.getStateId());
  ...
  }, [District, LBType, localbodies, Talukvalues, Villagevalues, PostOfficevalues, lbs, isInitialRender, cmbLB.length]);
