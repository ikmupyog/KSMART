...
const Search = ({ path }) => {
    ...
    if (window.location.href.includes("/birthsearch") == true && applicationType?.value == "birthsearch") {
        ...
        const { data: { CorrectionApplication: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useBirthCorrectionApplicationDetail({ tenantId, filter: payload })
