...
const Search = ({ path }) => {
  ...
  if (window.location.href.includes("/birthsearch") == true && applicationType?.value == "birthsearch") {
    ...
    const { data: { StillBirthChildDetails: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useSearchStillBirth({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { BornOutsideChildDetails: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useBornOutSideBirthSearch({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { nacDetails: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useBirthNACSearch({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { AbandonedDetails: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useAbandonedBirthSearch({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { deathCertificateDtls: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useSearchDeath({
      tenantId,
      filters: payload,
      config,
    });
    return (
      <SearchDeath
        ...
        tenantId={tenantId}
        onSubmit={onSubmit}
        setApplicationDeathType={setApplicationDeathType}
        applicationDeathType={applicationDeathType}
        data={!isLoading && isSuccess ? (searchResult?.length > 0 ? searchResult : { display: "ES_COMMON_NO_DATA" }) : ""}
        count={count}
      />
    );
  } else if (window.location.href.includes("/deathsearch") == true && applicationDeathType?.value == "deathnac") {
    const { data: { deathNACDtls: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useDeathNACSearch({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { deathAbandonedDtls: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useSearchAbandonedDeath({
      tenantId,
      filters: payload,
      config,
    });
    ...
    const { data: { deathCorrection: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useDeathCorrectionApplicationSearch({
      tenantId,
      filter: payload,
      config,
    });
    ...
    const {
      data: { CorrectionApplication: searchResult, Count: count } = {},
      isLoading,
      isSuccess,
    } = Digit.Hooks.cr.useBirthCorrectionApplicationSearch({ tenantId, filter: payload, config });
    ...
    console.log("initial");
    const {
      data: { MarriageDetails: searchResult, Count: count } = {},
      isLoading,
      isSuccess,
    } = Digit.Hooks.cr.useMarriageCorrectionApplicationDetail({ tenantId, filter: payload, config });
    return (
      <SearchMarriage
        t={t}
        tenantId={tenantId}
    ...
    console.log("initial");
    const { data: { MarriageDetails: searchResult, Count: count } = {}, isLoading, isSuccess } = Digit.Hooks.cr.useMarriageSearch({
      tenantId,
      filters: payload,
      config,
    });
