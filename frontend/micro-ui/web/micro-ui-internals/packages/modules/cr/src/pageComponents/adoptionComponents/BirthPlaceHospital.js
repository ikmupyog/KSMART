...
const BirthPlaceHospital = ({ config, onSelect, userType, formData, selectHospitalName, hospitalName, hospitalNameMl,
  selectHospitalNameMl, isEditAdoption
}) => {
  ...
  if (isEditAdoption) {
    if (formData?.AdoptionChildDetails?.hospitalCode != null) {
      if (cmbhospital.length > 0 && (hospitalName === undefined || hospitalName === "")) {
        selectHospitalName(cmbhospital.filter(cmbhospital => cmbhospital.code === formData?.AdoptionChildDetails?.hospitalCode)[0]);
        cmbhospitalMl = cmbhospital.filter(cmbhospital => cmbhospital.code === formData?.AdoptionChildDetails?.hospitalCode)[0];
  ...
  useEffect(() => {
    ...
    if (isInitialRender) {
      if (formData?.AdoptionChildDetails?.hospitalName) {
