...
const generateActions = (rowData) => {
  ...
  switch (status) {
    case "CITIZENACTIONREQUIRED":
      response = (
        <span className="link">
          <Link to={`/digit-ui/citizen/cr/cr-birth-creation/child-details/${rowData?.TL_COMMON_TABLE_COL_APP_NO}`}>EDIT</Link>
        </span>
      );
      break;
    case "PENDINGPAYMENT":
    ...
    default:
      response = (
        <span className="link">
          <Link to={`/digit-ui/citizen/cr/cr/application/${rowData?.TL_COMMON_TABLE_COL_APP_NO}/${rowData?.TL_COMMON_CITY_NAME}`}>VIEW DETAILS</Link>
        </span>
      );
      break;
  }
