...
const BrideAddressPermanent = ({ config, onSelect, userType, formData, permtaddressCountry, setpermtaddressCountry,
    ...
    useEffect(() => {
        if (isPrsentAddress && isInitialRender) {
            ...
            (formData?.BrideAddressDetails?.permtaddressStateName === null || formData?.BrideAddressDetails?.permtaddressStateName === "" || formData?.BrideAddressDetails?.permtaddressStateName === undefined)) {
            if (cmbLB.length > 0) {
                currentLB = cmbLB.filter((cmbLB) => cmbLB.code === tenantId);
                // setAdrsLBName(currentLB[0]);
                if (cmbCountry.length > 0 && currentLB.length > 0) {
