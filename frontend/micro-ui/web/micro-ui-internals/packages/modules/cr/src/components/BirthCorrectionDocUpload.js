...
const BirthCorrectionDocUpload = ({ title, showModal, onSubmit, hideModal, selectedConfig, selectedDocs, selectedDocData, selectedBirthData }) => {
  ...
  useEffect(() => {
    ...
    console.log(
      ...
      inclusionStudentCondition,
      Object.keys(inclusionStudentCondition)?.length > 0 && inclusionStudentCondition.code === "STUDENT"
    );
    ...
    if (
      (childAge < 6 && selectedConfig?.fieldName === "CHILD_NAME") ||
      Object.keys(inclusionCorrectionCondition)?.length > 0 ||
      Object.keys(inclusionStudentCondition)?.length > 0
    ) {
          ...
          <div>
            ...
            {showDatePicker && (
              <div>
                ...
                <CheckBox
                  label={t("ADD_HUSBAND_NAME_FOR_FEMALE")}
                  onChange={() => setCheckNameCorrectionCondition(!checkNameCorrectionCondition)}
                  value={checkNameCorrectionCondition}
                  checked={checkNameCorrectionCondition}
                />
              </div>
            )}
          </div>
