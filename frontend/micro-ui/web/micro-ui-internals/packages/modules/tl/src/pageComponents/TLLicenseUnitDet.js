...
const TLLicenseUnitDet = ({ t, config, onSelect, userType, formData }) => {
 ...
 const [mutationHappened,setmutationHappened]=useState(false);
  ...
  const [editview, setEditview] = useState(formDataPage ? Object?.keys(formDataPage)?.length === 0 ? false : true : false);
  if (tenantboundary) {
  ...
  const [payloadPT, setPayloadPT] = useState([]);
  ...
  const selectBusinessCategory = (i, value) => {
    ...
    units[i].businessCategory =  value ? value : null;
  ...
  const selectBusinessType = (i, value) => {
    let units = [...fields];
    let fillUnits = [...fillFields];
    units[i].businessType = value ? value : null;
  ...
  const selectBusinessSubType = (i, value) => {
    let units = [...fields];
    units[i].businessSubtype =  value ? value : null;
  ...
  let searchReultPT = "";
  let configPT = {
    enabled: !! ( dataPT !== undefined && mutationHappened === false)
  }
  let data1 = [];
        ...
        stateDoor.map((data, __index) => {
          ...
          if (searchReult?.length === 1) {
            ...
            return { ...data };
              ...
              setdataPT({
                "lbid": Localbody?.lbid,
                "zoneId" : zonalOffice?.code,
                "wardId" : WardNo?.code,
                "doorNo" : data?.doorNo,
                "doorNoSub" : data?.doorNoSub ? data?.doorNoSub : "00"
              });
          
           //   searchReultPT = mutationPT?.status === "success" && mutationPT?.isSuccess && !mutationPT?.isError ? mutationPT.data.BuildingDet : "";
  ...
  const mutationPT=Digit.Hooks.tl.useSearchPT(tenantId,configPT);
  //dataPT !== undefined  ? dataPT : {},
 //const{data: PTDoor = {}}=Digit.Hooks.tl.useSearchPT(dataPT !== undefined  ? dataPT : {},tenantId,configPT);
  //console.log(JSON.stringify(PTDoor));
  useEffect(() => {
    if ((dataPT) && (mutationPT !== undefined) && (structureType?.code === "BUILDING" ) && (mutationHappened === false)) {
      if (mutationPT?.error == null) {
        setmutationHappened(true);
        mutationPT.mutate(dataPT,{
          onSuccess,
        });
      }
    }
    
  }, [mutationPT,mutationHappened])
  ...
  if ((formDataPage?.tradeLicenseDetail?.tradeUnits || formDataPage?.tradeLicenseDetail?.structurePlace) && editview) {
    ...
    setBusinessCategoryMenu(BusinessCategoryMenutemp.filter((category) => category.categoryType === formDataPage?.tradeLicenseDetail?.businessSector.code));
    formDataPage?.tradeLicenseDetail?.tradeUnits?.map((unit)=>{
      category = unit?.businessCategory !== "" ? unit?.businessCategory : "";
      bustype = unit?.businessCategory !== "" && unit?.businessType !== "" ?  unit?.businessType : "";
      unit?.businessSubtype ? subType.push(unit?.businessSubtype) : "";       
    });
  ...
  const selectCategorysubtype = (e) => {
    ...
    for(let i=0; i<tradeUnitJSON.length; i++){
      ...
      subUnitSelect.length > 0 ? !subUnitSelect.find(subUnit => subUnit.code === tradeUnitJSON[i]?.businessSubtype.code) ? delete tradeUnitJSON[i] : null :null;
    }
  ...
  const goNext = async () => {
    ...
    if (result) {
      ...
      let ownerspremise = formDataPage?.tradeLicenseDetail?.ownerspremise ? formDataPage?.tradeLicenseDetail?.ownerspremise : [];
      let owners = formDataPage?.tradeLicenseDetail?.owners;
      
      if((mutationPT?.BuildingDet !== "") && (mutationPT?.BuildingDet !== null)){
        if((!ownerspremise)&&(ownerspremise !== []))
        ownerspremise = mutationPT?.BuildingDet?.buildingOwner ? mutationPT?.BuildingDet?.buildingOwner : [];
        if((!owners)&&(owners !== []))
        if(ownershipCategory === "OWN"){
          mutationPT?.BuildingDet?.buildingOwner ?
          mutationPT?.BuildingDet?.buildingOwner.map((owner)=>{
            owners.push({
              name: owner?.ownerName,
              applicantNameLocal: owner?.ownerNameLocal,
              careOf: "",
              careOfName: "",
              designation: "",
              houseName: owner?.houseName ? owner?.houseName : "",
              street: owner?.street ? owner?.street : "",
              locality: "",
              postOffice: "",
              pincode: owner?.pincode ? owner?.pincode : "",
              aadhaarNumber: owner?.aadhaarNumber ? owner?.aadhaarNumber : "",
              mobileNumber: owner?.mobileNumber ? owner?.mobileNumber : "",
              emailId: ""
            })
          })
           : [];
          ...
          <div style={{ borderRadius: "5px", borderColor: "#f3f3f3", background: "white", display: "flow-root", }} >
            ...
            <div className="row">
              ...
              <div className="col-md-3" ><CardLabel>{`${t("TL_LOCALIZATION_CAPITAL_AMOUNT")}`}&nbsp;(<svg style={{ display: "inline-block" }} class="icon icon-tabler icon-tabler-currency-rupee" width="15" height="15" viewBox="0 0 24 24" strokeWidth="1" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"> <path stroke="none" d="M0 0h24v24H0z" fill="none" /> <path d="M18 5h-11h3a4 4 0 0 1 0 8h-3l6 6" /> <line x1="7" y1="9" x2="18" y2="9" /> </svg>)<span className="mandatorycss">*</span></CardLabel>
                <TextInput t={t} isMandatory={config.isMandatory} type={"text"} optionKey="i18nKey" name="capitalInvestment" value={capitalInvestment} onChange={changesetCapitalInvestment}  {...(validation = { isRequired: true, title: t("TL_INVALID_CAPITAL_AMOUNT") })} />
              </div>
              <div className="col-md-3" >
                <CardLabel>{`${t("TL_NEW_TRADE_DETAILS_TRADE_COMM_DATE_LABEL")}`}<span className="mandatorycss">*</span></CardLabel>
                <DatePicker name="commencementDate" min={minDate} date={commencementDate} onChange={changesetCommencementDate} disabled={isEdit} {...(validation = { isRequired: true, title: t("TL_NEW_TRADE_DETAILS_TRADE_COMM_DATE_LABEL") })} />
              </div>
              {/* </div>
              </div>
              <div className="col-md-7" >
              ...
              <div className="col-md-3">
                ...
                <TextInput t={t} type={"text"} isMandatory={config.isMandatory} optionKey="i18nKey" name="noOfEmployees" value={noOfEmployees} onChange={changesetNoofEmployees} {...(validation = { pattern: "^[0-9`' ]*$", isRequired: true, type: "text", title: t("TL_INVALID_NO_EMPLOYEES"), })} />
              </div>
              {/* </div> */}
              {/* </div> */}
            </div>
            ...
            <div className="row">
              <div className="col-md-3" ><CardLabel>{`${t("TL_LICENSING_UNIT_NAME")}`}<span className="mandatorycss">*</span></CardLabel>
                <TextInput t={t} isMandatory={config.isMandatory} type={"text"} optionKey="i18nKey" name="licenseUnitName" value={licenseUnitName} onChange={changesetLicenseUnitName} disable={isEdit} {...(validation = { isRequired: true, type: "text", title: t("TL_INVALID_LICENSING_UNIT_NAME") })} />
              </div>
              <div className="col-md-3" ><CardLabel>{`${t("TL_LICENSING_UNIT_NAME_ML")}`}<span className="mandatorycss">*</span></CardLabel>
                <TextInput t={t} isMandatory={config.isMandatory} type={"text"} optionKey="i18nKey" name="licenseUnitNameLocal" value={licenseUnitNameLocal} onChange={changesetLicenseUnitNameLocal} disable={isEdit} {...(validation = { isRequired: true, type: "text", title: t("TL_INVALID_LICENSING_UNIT_NAME") })} />
              </div>
              <div className="col-md-3" ><CardLabel>{`${t("TL_CONTACT_NO")}`}<span className="mandatorycss">*</span></CardLabel>
                <TextInput t={t} isMandatory={config.isMandatory} type={"text"} optionKey="i18nKey" name="contactNo" value={contactNo} onChange={changesetContactNo} disable={isEdit} {...(validation = { pattern: "^[0-9]*$", isRequired: true, title: t("TL_INVALID_MOBILE_NO") })} />
              </div>
              <div className="col-md-3" ><CardLabel>{`${t("TL_LOCALIZATION_EMAIL_ID")}`}<span className="mandatorycss">*</span></CardLabel>
                <TextInput t={t} isMandatory={config.isMandatory} type="email" optionKey="i18nKey" name="email" value={email} onChange={changesetEmail} disable={isEdit} {...(validation = { isRequired: true, title: t("TL_INVALID_EMAIL_ID") })} />
              </div>
            </div>
            <div className="row">
