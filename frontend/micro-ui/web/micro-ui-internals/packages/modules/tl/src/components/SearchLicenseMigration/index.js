...
const SearchLicenseMigration = ({tenantId, t, onSubmit, data, count }) => {
    ...
    const handleLinkClick = (finaldata) => {
      Digit.SessionStorage.set("TL_MIGRATE_TRADE", finaldata);
    }
    ...
    const columns = useMemo( () => ([
        {
          ...
          Cell: ({ row }) => {
            return (
              <div>
                <span className="link">
                  <Link onClick={event => handleLinkClick(row.original)} to={{pathname:`/digit-ui/employee/tl/migrate-editapplication`}}>
...
export default SearchLicenseMigration