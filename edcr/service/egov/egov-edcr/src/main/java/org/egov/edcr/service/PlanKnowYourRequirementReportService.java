...
@Service
public class PlanKnowYourRequirementReportService extends PlanReportService {

	private static final Logger LOG = LogManager.getLogger(PlanKnowYourRequirementReportService.class);
    ...
    private Subreport getSub(ScrutinyDetail detail, int j, String title, String heading, String subheading,
        ...
        try {
            ...
            for (Integer s : detail.getColumnHeading().keySet()) {
                ...
                if (RULE_NO.equalsIgnoreCase(columnHeading.name)) {
                    columnWidth = byeLawColumnSize.intValue();
                }
                if (!STATUS.equalsIgnoreCase(columnHeading.name) && !PROVIDED.equalsIgnoreCase(columnHeading.name)
						&& !REMARKS.equalsIgnoreCase(columnHeading.name)) {
 ...
 /*   private Subreport getGeneralDetails() {
    ...
    public InputStream generateReport(Plan plan, EdcrApplication dcrApplication) {
        ...
        if (plan.getKyrerrors() != null && plan.getKyrerrors().size() > 0) {
			int i = 1;
			for (Map.Entry<String, String> entry : plan.getKyrerrors().entrySet()) {
        ...
        valuesMap.put("errors", plan.getKyrerrors());
        ...
        drb.setTemplateFile("/reports/templates/edcr_kyrr.jrxml");
