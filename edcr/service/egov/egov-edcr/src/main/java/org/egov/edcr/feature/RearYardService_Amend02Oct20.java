...
public class RearYardService_Amend02Oct20 extends GeneralRule {
    ...
    private Boolean checkRearYardLessThanTenMts(final Plan pl, final SetBack setback, final Building building,
            final Block block, final Integer level, final Plot plot, final String rearYardFieldName, final BigDecimal min,
            final BigDecimal mean, final OccupancyTypeHelper mostRestrictiveOccupancy, final RearYardResult rearYardResult,
        ...
        if (mostRestrictiveOccupancy.getType().getCode().equals(A1)
            ...
            this.compareRearYardResult(block.getName(), min, mean, mostRestrictiveOccupancy, rearYardResult, valid, subRule, rule,
                    minVal, meanVal, level);
        } else {
            valid = this.processRearYardForOccupanciesOtherThanA1A2F(pl, building, block, level, plot, rearYardFieldName,
                    subRuleDesc, min, mean, mostRestrictiveOccupancy, BigDecimal.ZERO, false, rearYardResult, buildingHeight);
        }
        return valid;
    }
    ...
    public Map<String, Date> getAmendments() {
        ...
        rearYardAmendments.put("amendment02Oct20", AmendmentConstants.AMEND_DATE_011020);
