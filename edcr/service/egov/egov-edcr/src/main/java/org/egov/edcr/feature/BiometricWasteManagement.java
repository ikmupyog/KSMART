	...
	public Plan validate(Plan pl) {
		...
		if (pl != null && pl.getVirtualBuilding() != null && !pl.getVirtualBuilding().getOccupancyTypes().isEmpty()) {
			for (OccupancyTypeHelper occupancyType : pl.getVirtualBuilding().getOccupancyTypes()) {
				if ((occupancyType.getType().getCode().equals(C) || occupancyType.getType().getCode().equals(C1)
					...
					errors.put(BIOMETRIC_WASTE_TREATMENT, edcrMessageSource.getMessage(OBJECTNOTDEFINED,
			...
			for (OccupancyTypeHelper occupancyType : pl.getVirtualBuilding().getOccupancyTypes()) {
				if ((occupancyType.getType().getCode().equals(C) || occupancyType.getType().getCode().equals(C1)
					...
					if (!pl.getUtility().getBiometricWasteTreatment().isEmpty()) {
						setReportOutputDetailsWithoutOccupancy(pl, subRule, SUBRULE_54_4_DESC, "", OBJECTDEFINED_DESC,
