...
public class TradeLicensePdeService {
    ...
    public List<TradeLicense> create(TradeLicenseRequest tradeLicenseRequest, String businessServicefromPath) {
        ...
        // tradeLicenseRequest.getLicenses().get(0).getTradeLicenseDetail().getOwners().get(0).setUuid("123456");
        // tradeLicenseRequest.getLicenses().get(0).getTradeLicenseDetail().getOwners().get(0)
        // .setId(Long.parseLong("123456"));
        /*
     ...
     * license.getTradeLicenseDetail().getOwners().stream()
     * .map(OwnerInfo::getMobileNumber).collect(Collectors.toList());
     * for (String mobno : mobileNumbers) {
     * TradeLicenseSearchCriteria tradeLicenseSearchCriteria =
     ...
     * List<TradeLicense> licensesFromSearch =
     * getLicensesFromMobileNumber(tradeLicenseSearchCriteria,
     * request.getRequestInfo());
     * List<String> tradeTypeResultforSameMobNo = new ArrayList<>();
     * for (TradeLicense result : licensesFromSearch) {
     * if (!StringUtils.equals(result.getApplicationNumber(),
     * license.getApplicationNumber())
     * && !StringUtils.equals(result.getStatus(), STATUS_REJECTED)) {
     * tradeTypeResultforSameMobNo.add(result.getTradeLicenseDetail().getTradeUnits(
     * ).get(0)
     ...
     * 
     * if (rejectedLicense.getStatus().toString().equalsIgnoreCase(TLConstants.
     * STATUS_REJECTED)) {
     * TradeLicenseSearchCriteria rejectedCriteria = new
     * TradeLicenseSearchCriteria();
     * 
     ...
     * 
     * List<TradeLicense> rejectedLicenses =
     * getLicensesWithOwnerInfo(rejectedCriteria, requestInfo);
     * 
     * TradeLicense latestApplication = rejectedLicense;
     * 
     * for (TradeLicense newLicense : rejectedLicenses) {
     * if
     * (latestApplication.getStatus().equalsIgnoreCase(TLConstants.STATUS_REJECTED))
     * {
     ...
     * }
     * 
     * private void filterRejectedApplications(RequestInfo requestInfo,
     * List<TradeLicense> licenses) {
     * String currentFinancialYear = "";
     ...
     * licenses.removeIf(t ->
     * t.getStatus().toString().equalsIgnoreCase(TLConstants.STATUS_REJECTED)
     * && !t.getFinancialYear().toString().equalsIgnoreCase(checker));
     * 
     * }
     * 
     * public int countLicenses(TradeLicenseSearchCriteria criteria, RequestInfo
     * requestInfo, String serviceFromPath,
     * HttpHeaders headers) {
     * 
     * criteria.setBusinessService(serviceFromPath);
     * enrichmentService.enrichSearchCriteriaWithAccountId(requestInfo, criteria);
     ...
     * }
     * 
     * public void checkEndStateAndAddBPARoles(TradeLicenseRequest
     * tradeLicenseRequest) {
     ...
     * license.getStatus().equalsIgnoreCase(endstates.get(i))) {
     * licensesToAddRoles.add(license);
     * }
     * }
     * if (!licensesToAddRoles.isEmpty()) {
     * TradeLicenseRequest tradeLicenseRequestForUserUpdate =
     * TradeLicenseRequest.builder()
     * .licenses(licensesToAddRoles).requestInfo(tradeLicenseRequest.getRequestInfo(
     * )).build();
    ...
    /*
     * public List<TradeLicense>
     * getLicensesFromMobileNumber(TradeLicenseSearchCriteria criteria,
     * RequestInfo requestInfo) {
     * 
     ...
     */
    /*
     * public List<TradeLicense> getLicensesWithOwnerInfo(TradeLicenseSearchCriteria
     * criteria, RequestInfo requestInfo) {
     ...
     * if (!license.getApplicationNumber().equalsIgnoreCase(duplicateLicense.
     * getApplicationNumber())
     * &&
     * license.getLicenseNumber().equalsIgnoreCase(duplicateLicense.getLicenseNumber
     * ())
     * && duplicateLicense.getFinancialYear().compareTo(license.getFinancialYear())
     * < 0) {
     ...
     * licenses.removeIf(t ->
     * t.getApplicationNumber().equalsIgnoreCase(duplicateLicense.
     * getApplicationNumber()));
     * }
     * 
     * }
     ...
     */
    /*
     * public List<TradeLicense> getLicensesWithOwnerInfo(TradeLicenseRequest
     * request) {
    ...
    /*
     * private void validateLatestApplicationCancellation(TradeLicenseRequest
     * tradeLicenseRequest,
     ...
     * 
     * List<TradeLicense> searchResultForCancellation =
     * getLicensesWithOwnerInfo(criteria,
     * tradeLicenseRequest.getRequestInfo());
     * 
     * actionValidator.validateUpdateRequest(tradeLicenseRequest, businessService,
     * searchResultForCancellation);
     * 
     * }
     */

    /*
     * public List<TradeLicense> plainSearch(TradeLicenseSearchCriteria criteria,
     * RequestInfo requestInfo) {
     * List<TradeLicense> licenses;
     ...
     * criteria.setIds(ids);
     * 
     * TradeLicenseSearchCriteria idsCriteria =
     * TradeLicenseSearchCriteria.builder().ids(ids).build();
     * 
     ...
     */
    /*
     * public void runJob(String serviceName, String jobname, RequestInfo
     * requestInfo) {
     * 
     * if (serviceName == null)
     * serviceName = TRADE_LICENSE_MODULE_CODE;
     * 
     * tlBatchService.getLicensesAndPerformAction(serviceName, jobname,
     * requestInfo);
     * 
     * }
     */
    ...
    /*
     * public boolean enrichWithUserDetails(TradeLicenseSearchCriteria criteria,
     * RequestInfo requestInfo) {
     * List<TradeLicense> licenses = new LinkedList<>();
     ...
     * }
     * 
     * Boolean isRenewalPending = (criteria.getRenewalPending() != null &&
     * criteria.getRenewalPending() == true);
     * 
