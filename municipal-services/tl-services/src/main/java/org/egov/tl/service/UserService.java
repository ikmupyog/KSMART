...
public class UserService {
    ...
    public UserService(ObjectMapper mapper, ServiceRequestRepository serviceRequestRepository, TLConfiguration config,
            TradeUtil tradeUtil, TLRepository repository) {
        this.mapper = mapper;
            ...
            tradeLicense.getTradeLicenseDetail().getOwners().forEach(owner -> {
                ...
                switch (businessService) {
                    case businessService_BPA:
                        UserDetailResponse userDetailResponse = searchByUserName(owner.getMobileNumber(),
                ...
                if (owner.getUuid() == null) {
                    ...
                    if (isBPARoleAddRequired) {
                        ...
                        for (String rolename : licenseeTyperRole) {
                            user.addRolesItem(Role.builder().code(rolename).name(rolename)
                                    .tenantId(tradeLicense.getTenantId()).build());
                        }
                    }
    ...
    public void createUserForCorrection(CorrectionRequest request) {
        List<Correction> licenses = request.getLicenseCorrection();
        RequestInfo requestInfo = request.getRequestInfo();
        Role role = getCitizenRole(licenses.get(0).getTenantId());

        licenses.forEach(correction -> {
            correction.getOwnerAddUpdate().forEach(owner -> {
                String businessService = businessService_TL;
                if (owner.getUuid() == null) {
                    addUserDefaultFields(correction.getTenantId(), role, owner, businessService);
                    // UserDetailResponse userDetailResponse = userExists(owner,requestInfo);
                    StringBuilder uri = new StringBuilder(config.getUserHost())
                            .append(config.getUserContextPath())
                            .append(config.getUserCreateEndpoint());
