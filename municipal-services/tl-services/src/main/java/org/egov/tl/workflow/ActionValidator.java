...
public class ActionValidator {
        ...
        request.getLicenses().forEach(license -> {
            ...
            switch (businessService) {
                case businessService_TL:
                    ...
                    // errorMap.put("INVALID ACTION", "Action should be APPLY when application
        ...
        for (TradeLicense license : licenses) {
            ...
            if (license.getAction() != null && license.getAction().toString().equalsIgnoreCase(ACTION_CANCEL)) {
                ...
                for (TradeLicense searchedLicense : searchResult) {
                    if (searchedLicense.getLicenseNumber().toString()
                            .equalsIgnoreCase(license.getLicenseNumber().toString())) {
                ...
                for (int i = 0; i < existingApplications.size(); i++) {
                    ...
                    if (!existingApplications.get(i).getApplicationNumber().toString()
                            .equalsIgnoreCase(license.getApplicationNumber().toString())
                            && !existingApplications.get(i).getStatus().toString().equalsIgnoreCase(STATUS_CANCELLED)
                            && license.getFinancialYear().toString()
                                    .compareTo(existingApplications.get(i).getFinancialYear().toString()) < 0) {
                        errorMap.put("INVALID_ACTION",
                                "Cannot cancel an application when later applications are in the workflow");
                        break;
                    }
                }
    ...
    private void validateIds(TradeLicenseRequest request, BusinessService businessService) {
        ...
        request.getLicenses().forEach(license -> {
            ...
            if ((namefBusinessService == null) || (namefBusinessService.equals(businessService_TL))
                    || (namefBusinessService.equals(businessService_BPA)
