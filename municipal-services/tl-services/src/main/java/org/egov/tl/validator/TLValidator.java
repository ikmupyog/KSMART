...
public class TLValidator {
    ...
    public void validateBusinessService(TradeLicenseRequest request, String businessServiceFromPath) {
        ...
        if (!allowedservices.contains(businessServiceFromPath)) {
            throw new CustomException("BUSINESSSERVICE_NOTALLOWED",
                    " The business service is not allowed in this module");
        }
        for (TradeLicense license : request.getLicenses()) {
            String licenseBusinessService = license.getBusinessService() == null ? businessService_TL
                    : license.getBusinessService();
            ...
            license.getTradeLicenseDetail().getOwners().forEach(
                    owner -> {
                        ...
                        if (owner.getPermanentAddress() == null)
                            throw new CustomException("NULL_PERMANENTADDRESS",
                                    " User Permanent Address cannot be null");
                        //
                        // if (owner.getCorrespondenceAddress() == null)
                        // throw new CustomException("NULL_CORRESPONDANCEADDRESS", " User Correspondance
    ...
    private void valideDates(TradeLicenseRequest request, Object mdmsData) {
        request.getLicenses().forEach(license -> {
            ...
            // license.getApplicationType().toString().equals(TLConstants.APPLICATION_TYPE_RENEWAL)){
            // taxPeriods = tradeUtil.getTaxPeriods(license,mdmsData);
            // }else{
            // taxPeriods = tradeUtil.getTaxPeriods(license,mdmsData);
            // }
            taxPeriods = tradeUtil.getTaxPeriods(license, mdmsData);
            // if (license.getValidTo() != null && license.getValidTo() >
            // taxPeriods.get(TLConstants.MDMS_ENDDATE)) {
            // Date expiry = new Date(license.getValidTo());
            // throw new CustomException("INVALID TO DATE", " Validto cannot be greater
            // than: " + expiry);
            // }
            if (license.getLicenseType().toString()
                    .equalsIgnoreCase(TradeLicense.LicenseTypeEnum.TEMPORARY.toString())) {
                ...
                if (!config.getIsPreviousTLAllowed() && license.getValidFrom() != null
                    ...
                    throw new CustomException("INVALID FROM DATE",
                            "The validFrom date cannot be less than CurrentDate");
    ...
    public void validateRenewal(TradeLicenseRequest request) {
        ...
        request.getLicenses().forEach(license -> {
            if (license.getStatus().equalsIgnoreCase(TLConstants.STATUS_MANUALLYEXPIRED)
                ...
                throw new CustomException("LICENSE MANUALLY EXPIRED",
                        "Licenses which are manually expired cannot be renewed");
            }
        });
            ...
            if (license.getApplicationType() != null
                ...
                if (license.getStatus().equalsIgnoreCase(TLConstants.STATUS_APPROVED)
                    ...
                    if (currentFromDate < existingToDate) {
                        throw new CustomException("INVALID FROM DATE",
                                "ValidFrom should be greater than the previous applications ValidTo Date");
                    }
                    if (currentFromDate <= existingFromDate) {
                        throw new CustomException("INVALID FROM DATE",
                                "ValidFrom should be greater than the applications ValidFrom Date");
                    }
                    if (currentToDate <= existingToDate) {
                        throw new CustomException("INVALID TO DATE",
                                "ValidTo should be greater than the applications ValidTo Date");
                    }
                    ...
                    throw new CustomException("RENEWAL ERROR",
                            "The license applied for renewal is not present in the repository");
                }
            }
        });
    }
        ...
        for (TradeLicense license : licenses) {
            ...
            if (!flag)
                errorMap.put("INVALID UPDATE",
                        "All TradeUnits are inactive in the tradeLicense: " + license.getApplicationNumber());
        }
    ...
    /**
     * Returns the list of ids of all applicationDoc as list for the given
     * tradelicense
     * 
    ...
    /**
     * Returns the list of ids of all verficationDoc as list for the given
     * tradelicense
     * 
    ...
    private void setFieldsFromSearch(TradeLicenseRequest request, List<TradeLicense> searchResult, Object mdmsData) {
        ...
        request.getLicenses().forEach(license -> {
            license.getAuditDetails()
                    .setCreatedBy(idToTradeLicenseFromSearch.get(license.getId()).getAuditDetails().getCreatedBy());
            license.getAuditDetails()
                    .setCreatedTime(idToTradeLicenseFromSearch.get(license.getId()).getAuditDetails().getCreatedTime());
            ...
            switch (businessService) {
                case businessService_TL:
                    if (!idToTradeLicenseFromSearch.get(license.getId()).getFinancialYear()
                            .equalsIgnoreCase(license.getFinancialYear())
    ...
    private void validateAllIds(List<TradeLicense> searchResult, List<TradeLicense> licenses) {
        ...
        licenses.forEach(license -> {
            ...
            if (!searchedLicense.getApplicationNumber().equalsIgnoreCase(license.getApplicationNumber()))
                errorMap.put("INVALID UPDATE",
                        "The application number from search: " + searchedLicense.getApplicationNumber()
        ...
        request.getLicenses().forEach(license -> {
            ...
            if (!flag)
                errorMap.put("INVALID OWNER",
                        "All owners are inactive for application:  " + license.getApplicationNumber());
        });
        if (!errorMap.isEmpty())
            throw new CustomException(errorMap);
    }
