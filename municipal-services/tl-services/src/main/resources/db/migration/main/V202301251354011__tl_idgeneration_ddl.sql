...
BEGIN
genid := (Select lastid From eg_idgeneration where tenantid = _tenantid AND yearid = _yearid AND idtype=_idtype AND modulecode = _modulecode) ;
IF ( genid IS NOT NULL ) THEN
	genid :=  genid  + 1 ;
	Update eg_idgeneration SET lastid = genid
		WHERE tenantid = _tenantid AND yearid = _yearid AND idtype=_idtype AND modulecode = _modulecode;
ELSE
	genid := 1  ;
	INSERT INTO eg_idgeneration(idtype, modulecode, tenantid, yearid, lastid)
