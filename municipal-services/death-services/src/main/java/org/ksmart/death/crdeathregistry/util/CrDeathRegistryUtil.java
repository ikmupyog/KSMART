...
@Component
public class CrDeathRegistryUtil {

    private ServiceRequestRepository serviceRequestRepository;
    private CrDeathRegistryConfiguration config;

    @Autowired
    public CrDeathRegistryUtil(CrDeathRegistryConfiguration config, ServiceRequestRepository serviceRequestRepository) {
        this.config = config;
    ...
    private ModuleDetail getTenantIdRequest() {
       ...
       // final String filterCodeForUom = "$.[?(@.tenantId=='kl' && @.moduleName=='tenant')]";
        ...
        crDeathMasterDetails
                .add(MasterDetail.builder().name(CrDeathRegistryConstants.TENANTS).build());
